...
(() => {
  ...
  /** Turnstile token for the form being submitted, if the widget is present. */
  const turnstileToken = (form) => {
    const el = form.querySelector('.cf-turnstile');
    if (!el || typeof turnstile === 'undefined') return '';
    try { return turnstile.getResponse(el) || ''; } catch { return ''; }
  };
  const resetTurnstile = (form) => {
    const el = form.querySelector('.cf-turnstile');
    if (el && typeof turnstile !== 'undefined') { try { turnstile.reset(el); } catch { /* ignore */ } }
  };
    ...
    dform.addEventListener('submit', async (e) => {
      ...
      try {
        const r = await fetch('/api/checkout', {
          ...
          body: JSON.stringify({ amount, frequency, turnstileToken: turnstileToken(dform) }),
        });
  ...
  if (aform) {
    ...
    aform.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        resetTurnstile(aform);   // tokens are single-use, success or not
        if (r.ok) {
          aform.reset();
          ...
          show(status, 'err', d.error === 'verification_failed'
